<script setup>
import { ref, reactive, watch, onMounted } from "vue";
import {
  getUsers,
  delUser,
  putUser,
  chkId,
  registUser,
  getUser,
  chkNickName,
} from "@/api/member";
import { Modal } from "bootstrap";

const users = ref([]);

const formRef = ref();
const registRef = ref();

const userInfo = ref({});

const modal = reactive({
  modal: null,
});

const columns = ref([
  {
    title: "회원번호",
    dataIndex: "userNo",
    key: "userNo",
    align: "center",
  },
  {
    title: "아이디",
    dataIndex: "userId",
    key: "userId",
    align: "center",
  },
  {
    title: "이름",
    dataIndex: "userName",
    key: "userName",
    align: "center",
  },
  {
    title: "닉네임",
    dataIndex: "nickName",
    key: "nickName",
    align: "center",
  },
  {
    title: "가입일",
    dataIndex: "joinDate",
    key: "joinDate",
    align: "center",
  },
  {
    title: "보기",
    key: "operation",
    align: "center",
  },
]);

watch(
  userInfo,
  () => {
    formState.userNo = userInfo.value.userNo;
    formState.userId = userInfo.value.userId;
    formState.userPass = userInfo.value.userPass;
    formState.userName = userInfo.value.userName;
    formState.nickName = userInfo.value.nickName;
    formState.sex = userInfo.value.sex;
    formState.birthDate = userInfo.value.birthDate;
    if (userInfo.value.emailId) {
      formState.emailId = userInfo.value.emailId;
    }
    if (userInfo.value.emailDomain) {
      formState.emailDomain = userInfo.value.emailDomain;
    }
    if (userInfo.value.phoneNumber1) {
      formState.phoneNumber1 = userInfo.value.phoneNumber1;
    }
    if (userInfo.value.phoneNumber2) {
      formState.phoneNumber2 = userInfo.value.phoneNumber2;
    }
    if (userInfo.value.phoneNumber3) {
      formState.phoneNumber3 = userInfo.value.phoneNumber3;
    }
  },
  { deep: true }
);

onMounted(() => {
  getList();
  modal.modal = new Modal("#userViewModal", {});
});

const labelCol = {
  style: {
    width: "150px",
  },
};

const wrapperCol = {
  span: 14,
};

const formState = reactive({
  userNo: 0,
  userId: "",
  userPass: "",
  checkPass: "",
  userName: "",
  nickName: "",
  sex: "male",
  birthDate: "",
  emailId: "",
  emailDomain: "",
  phoneNumber1: "010",
  phoneNumber2: "",
  phoneNumber3: "",
});

const registState = reactive({
  userId: "",
  userPass: "",
  checkPass: "",
  userName: "",
  nickName: "",
  sex: "male",
  birthDate: "",
  emailId: "",
  emailDomain: "",
  phoneNumber1: "010",
  phoneNumber2: "",
  phoneNumber3: "",
});

const validateNickName = async (_rule, value) => {
  if (userInfo.value.nickName == formState.nickName) {
    return Promise.resolve();
  }

  if (value === "") {
    return Promise.reject("닉네임을 입력하세요.");
  } else {
    if (formState.nickName.length < 2 || formState.nickName.length > 16) {
      return Promise.reject("닉네임은 2자 이상 16자 이하입니다.");
    } else {
      let promise = new Promise((resolve, reject) => {
        chkNickName(formState.nickName, ({ data }) => {
          if (data === 0) {
            resolve();
          } else {
            reject("닉네임이 존재합니다.");
          }
        });
      });

      return await promise;
    }
  }
};

const validatePass = async (_rule, value) => {
  if (formState.userPass.length === 0) {
    return Promise.resolve();
  }

  if (formState.userPass === userInfo.value.userPass) {
    return Promise.resolve();
  }

  if (formState.userPass.length < 6 || formState.userPass.length > 16) {
    return Promise.reject("비밀번호는 6자 이상 16자 이하입니다.");
  }

  return Promise.resolve();
};

const validateIdRegist = async (_rule, value) => {
  if (value === "") {
    return Promise.reject("아이디를 입력하세요.");
  } else {
    if (registState.userId.length < 5 || registState.userId.length > 16) {
      return Promise.reject("아이디는 5자 이상 16자 이하입니다.");
    } else {
      let promise = new Promise((resolve, reject) => {
        chkId(registState.userId, ({ data }) => {
          if (data === 0) {
            resolve();
          } else {
            reject("아이디가 존재합니다.");
          }
        });
      });

      return await promise;
    }
  }
};

const validateNameRegist = async (_rule, value) => {
  if (value === "") {
    return Promise.reject("이름을 입력하세요.");
  } else {
    if (registState.userName.length < 2 || registState.userName.length > 16) {
      return Promise.reject("이름은 2자 이상 16자 이하입니다.");
    } else {
      return Promise.resolve();
    }
  }
};

const validateNickNameRegist = async (_rule, value) => {
  if (value === "") {
    return Promise.reject("닉네임을 입력하세요.");
  } else {
    if (registState.nickName.length < 2 || registState.nickName.length > 16) {
      return Promise.reject("닉네임은 2자 이상 16자 이하입니다.");
    } else {
      let promise = new Promise((resolve, reject) => {
        chkNickName(registState.nickName, ({ data }) => {
          if (data === 0) {
            resolve();
          } else {
            reject("닉네임이 존재합니다.");
          }
        });
      });

      return await promise;
    }
  }
};

const validatePassRegist = async (_rule, value) => {
  if (value === "") {
    return Promise.reject("비밀번호를 입력하세요.");
  } else {
    if (registState.userPass.length < 6 || registState.userPass.length > 16) {
      return Promise.reject("비밀번호는 6자 이상 16자 이하입니다.");
    }
    if (registState.checkPass !== "") {
      registRef.value.validateFields("checkPass");
    }
    return Promise.resolve();
  }
};

const validatePassRegist2 = async (_rule, value) => {
  if (value === "") {
    return Promise.reject("비밀번호를 다시 입력하세요.");
  } else if (value !== registState.userPass) {
    return Promise.reject("비밀번호가 일치하지 않습니다.");
  } else {
    return Promise.resolve();
  }
};

const rules = {
  userPass: [
    {
      validator: validatePass,
      trigger: "change",
    },
  ],
  nickName: [
    {
      required: true,
      validator: validateNickName,
      trigger: "change",
    },
  ],
  sex: [
    {
      required: true,
    },
  ],
};

const registRules = {
  userId: [
    {
      required: true,
      validator: validateIdRegist,
      trigger: "change",
    },
  ],
  userPass: [
    {
      required: true,
      validator: validatePassRegist,
      trigger: "change",
    },
  ],
  checkPass: [
    {
      required: true,
      validator: validatePassRegist2,
      trigger: "change",
    },
  ],
  userName: [
    {
      required: true,
      validator: validateNameRegist,
      trigger: "change",
    },
  ],
  nickName: [
    {
      required: true,
      validator: validateNickNameRegist,
      trigger: "change",
    },
  ],
  sex: [
    {
      required: true,
    },
  ],
};

const onSubmit = () => {
  // console.log(formState.birthDate);
  formRef.value.validate().then(() => {
    // console.log(formState);
    putUser(
      formState,
      ({ status }) => {
        if (status === 200) {
          alert("수정이 완료되었습니다.");
          getList();
        }
      },
      (err) => {
        alert("오류가 발생했습니다.");
        console.log(err);
      }
    );
  });
};

const deleteUser = () => {
  if (confirm("정말로 삭제하겠습니까?")) {
    delUser(userInfo.value.userNo, ({ status }) => {
      if (status === 200) {
        alert("삭제가 완료되었습니다.");
        closeModal();
        getList();
      }
    });
  }
};

const fail = (error) => console.log(JSON.stringify(error));

const getList = () => {
  const success = (response) => {
    if (response.status == 200) {
      users.value = response.data;
    }
  };

  getUsers(success, fail);
};

const userView = (userNo) => {
  const success = (response) => {
    if (response.status == 200) {
      userInfo.value = response.data;
      // console.log(selectedUser.value);
      modal.modal.show();
    }
  };

  getUser(userNo, success, fail);
};

const closeModal = () => {
  registRef.value.resetFields();
  modal.modal.hide();
};

const regist = () => {
  registRef.value.validate().then(() => {
    registUser(
      registState,
      ({ status }) => {
        if (status === 200) {
          alert("회원 등록이 완료되었습니다.");
          getList();
          registState.userId = "";
          registState.userPass = "";
          registState.checkPass = "";
          registState.userName = "";
          registState.nickName = "";
          registState.sex = "male";
          registState.birthDate = "";
          registState.emailId = "";
          registState.emailDomain = "";
          registState.phoneNumber1 = "010";
          registState.phoneNumber2 = "";
          registState.phoneNumber3 = "";

          document.querySelector("#closeBtn").click();
        }
      },
      (err) => {
        alert("오류가 발생했습니다.");
        console.log(err);
      }
    );
  });
};
</script>

<template>
  <div class="gray-head">
    <h2 class="mt-4 py-3 shadow-sm bg-light text-center">회원목록</h2>
  </div>
  <div class="row justify-content-center">
    <div class="col-lg-8 col-md-10 col-sm-12">
      <div class="row align-self-center mb-2">
        <div class="col-md-2 text-start">
          <button
            class="btn btn-outline-primary btn-sm"
            style="margin-top: 50px"
            data-bs-toggle="modal"
            data-bs-target="#registerModal">
            회원등록
          </button>
        </div>
      </div>
      <a-table
        :data-source="users"
        :columns="columns"
        :pagination="{ position: ['bottomCenter'] }">
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'operation'">
            <a @click="userView(record.userNo)">보기</a>
          </template>
        </template>
      </a-table>
    </div>
  </div>

  <!-- 회원 등록 Modal -->
  <div
    class="modal fade"
    id="registerModal"
    data-bs-backdrop="static"
    data-bs-keyboard="false"
    tabindex="-1"
    aria-labelledby="staticBackdropLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="ms-3 modal-title">회원 등록</h5>

          <button
            id="closeBtn"
            type="button"
            class="btn-close"
            data-bs-dismiss="modal"
            aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <a-form
            ref="registRef"
            :model="registState"
            :label-col="labelCol"
            :wrapper-col="wrapperCol"
            :rules="registRules"
            layout="horizontal"
            style="max-width: 600px">
            <a-form-item has-feedback label="아이디" name="userId">
              <a-input v-model:value="registState.userId" autocomplete="off" />
            </a-form-item>

            <a-form-item has-feedback label="비밀번호" name="userPass">
              <a-input
                v-model:value="registState.userPass"
                type="password"
                autocomplete="off" />
            </a-form-item>

            <a-form-item has-feedback label="비밀번호 확인" name="checkPass">
              <a-input
                v-model:value="registState.checkPass"
                type="password"
                autocomplete="off" />
            </a-form-item>

            <a-form-item has-feedback label="이름" name="userName">
              <a-input
                v-model:value="registState.userName"
                autocomplete="off" />
            </a-form-item>

            <a-form-item has-feedback label="닉네임" name="nickName">
              <a-input
                v-model:value="registState.nickName"
                autocomplete="off"
                ref="refNn" />
            </a-form-item>

            <a-form-item label="성별" name="sex">
              <a-radio-group v-model:value="registState.sex">
                <a-radio value="male">남자</a-radio>
                <a-radio value="female">여자</a-radio>
              </a-radio-group>
            </a-form-item>

            <a-form-item has-feedback label="생년월일" name="birthDate">
              <a-date-picker
                v-model:value="registState.birthDate"
                value-format="YYYY-MM-DD" />
            </a-form-item>

            <a-form-item label="이메일">
              <a-input-group compat>
                <a-input
                  name="emailId"
                  v-model:value="registState.emailId"
                  style="width: 43%; text-align: center" />
                <a-input
                  class="site-input-split"
                  style="width: 13%; pointer-events: none; text-align: center"
                  placeholder="@"
                  disabled />
                <a-input
                  name="emailDomain"
                  v-model:value="registState.emailDomain"
                  style="width: 44%; text-align: center" />
              </a-input-group>
            </a-form-item>

            <a-form-item label="전화번호">
              <a-input-group compact>
                <a-select
                  v-model:value="registState.phoneNumber1"
                  style="width: 30%; text-align: center">
                  <a-select-option value="010">010</a-select-option>
                  <a-select-option value="011">011</a-select-option>
                  <a-select-option value="016">016</a-select-option>
                  <a-select-option value="017">017</a-select-option>
                  <a-select-option value="019">019</a-select-option>
                </a-select>
                <a-input-number
                  name="phoneNumber2"
                  v-model:value="registState.phoneNumber2"
                  style="width: 30%; text-align: center" />
                <a-input
                  class="site-input-split"
                  style="width: 10%; pointer-events: none; text-align: center"
                  placeholder="-"
                  disabled />
                <a-input-number
                  name="phoneNumber3"
                  v-model:value="registState.phoneNumber3"
                  style="width: 30%; text-align: center" />
              </a-input-group>
            </a-form-item>

            <a-form-item style="text-align: right">
              <a-button size="large" @click="regist">회원가입</a-button>
            </a-form-item>
          </a-form>
        </div>
      </div>
    </div>
  </div>

  <!-- 회원 정보 모달 -->
  <div
    class="modal fade"
    id="userViewModal"
    backdrop="static"
    keyboard="false"
    tabindex="-1"
    aria-labelledby="staticBackdropLabel"
    aria-hidden="true"
    ref="modal">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="ms-3 modal-title">회원 정보</h5>

          <button
            type="button"
            class="btn-close"
            data-bs-dismiss="modal"
            aria-label="Close"
            @click="closeModal"></button>
        </div>
        <div class="modal-body">
          <a-form
            ref="formRef"
            :model="formState"
            :label-col="labelCol"
            :wrapper-col="wrapperCol"
            :rules="rules"
            layout="horizontal"
            style="max-width: 600px">
            <a-form-item has-feedback label="아이디" name="userId">
              <a-input
                v-model:value="formState.userId"
                autocomplete="off"
                disabled
                style="
                  background-color: white;
                  color: black;
                  pointer-events: none;
                " />
            </a-form-item>

            <a-form-item has-feedback label="비밀번호" name="userPass">
              <a-input v-model:value="formState.userPass" autocomplete="off" />
            </a-form-item>

            <a-form-item has-feedback label="이름" name="userName">
              <a-input
                v-model:value="formState.userName"
                autocomplete="off"
                disabled
                style="
                  background-color: white;
                  color: black;
                  pointer-events: none;
                " />
            </a-form-item>

            <a-form-item has-feedback label="닉네임" name="nickName">
              <a-input
                v-model:value="formState.nickName"
                autocomplete="off"
                ref="refNn" />
            </a-form-item>

            <a-form-item label="성별" name="sex">
              <a-radio-group v-model:value="formState.sex">
                <a-radio value="male">남자</a-radio>
                <a-radio value="female">여자</a-radio>
              </a-radio-group>
            </a-form-item>

            <a-form-item has-feedback label="생년월일" name="birthDate">
              <a-date-picker
                v-model:value="formState.birthDate"
                value-format="YYYY-MM-DD" />
            </a-form-item>

            <a-form-item label="이메일">
              <a-input-group compat>
                <a-input
                  name="emailId"
                  v-model:value="formState.emailId"
                  style="width: 43%; text-align: center" />
                <a-input
                  class="site-input-split"
                  style="width: 13%; pointer-events: none; text-align: center"
                  placeholder="@"
                  disabled />
                <a-input
                  name="emailDomain"
                  v-model:value="formState.emailDomain"
                  style="width: 44%; text-align: center" />
              </a-input-group>
            </a-form-item>

            <a-form-item label="전화번호">
              <a-input-group compact>
                <a-select
                  v-model:value="formState.phoneNumber1"
                  style="width: 30%; text-align: center">
                  <a-select-option value="010">010</a-select-option>
                  <a-select-option value="011">011</a-select-option>
                  <a-select-option value="016">016</a-select-option>
                  <a-select-option value="017">017</a-select-option>
                  <a-select-option value="019">019</a-select-option>
                </a-select>
                <a-input-number
                  name="phoneNumber2"
                  v-model:value="formState.phoneNumber2"
                  style="width: 30%; text-align: center" />
                <a-input
                  class="site-input-split"
                  style="width: 10%; pointer-events: none; text-align: center"
                  placeholder="-"
                  disabled />
                <a-input-number
                  name="phoneNumber3"
                  v-model:value="formState.phoneNumber3"
                  style="width: 30%; text-align: center" />
              </a-input-group>
            </a-form-item>

            <a-form-item
              v-if="userInfo.userId != 'admin'"
              :wrapper-col="{ offset: 8, span: 18 }">
              <a-button class="me-3" size="large" @click="onSubmit">
                수정
              </a-button>
              <a-button size="large" @click="deleteUser">삭제</a-button>
            </a-form-item>

            <a-form-item
              v-if="userInfo.userId == 'admin'"
              :wrapper-col="{ offset: 10, span: 18 }">
              <a-button class="me-3" size="large" @click="onSubmit">
                수정
              </a-button>
            </a-form-item>
          </a-form>
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped></style>
